...
import PIL
# import google.generativeai as genai  # Commented out - no API key
...
model_path = 'model_trained_101class.hdf5'  # Look in current directory
try:
    model_best = load_model(model_path, compile=False)
    print('model successfully loaded!')
except Exception as e:
    print(f'Model not found at {model_path}: {e}')
    print('Please download the trained model and place it in the current directory')
    print('Or update the model path in line 131')
    exit(1)
...
# Make prediction
        ...
        pa['ai'] = f"AI analysis not available. Detected food: {_true}"
