...
export default function MealPlannerScreen({ navigation }) {
    ...
    const handleAdd = (key) => { const updated = addToPlate(key); setPlate(updated); setRecs(getRecommendations()); };
    const handleRemove = (key) => { const updated = removeFromPlate(key); setPlate(updated); setRecs(getRecommendations()); };
    const handleClear = () => { const updated = clearPlate(); setPlate(updated); setRecs([]); };
            ...
            <View style={[styles.menuCard, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: radius.md }]}>
                <View style={{ flex: 1 }}>
                    ...
                    <Text style={[typography.small, { color: colors.textMuted, marginTop: 2 }]}>
                        {item.calories} cal • {item.protein}g protein • ₹{item.price}
                    </Text>
                </View>
                ...
                ListHeaderComponent={() => (
                    <View>
                        ...
                        <View style={[styles.actionsRow, { marginTop: spacing.md, marginBottom: spacing.md }]}>
                            <TouchableOpacity
                                style={[styles.actionBtn, { backgroundColor: showRecs ? colors.cardAlt : colors.accent, borderRadius: radius.md }]}
                                onPress={() => { setShowRecs(!showRecs); setRecs(getRecommendations()); }}
                            >
                                <Text style={[typography.caption, { color: showRecs ? colors.text : '#FFF' }]}>{showRecs ? '📋 Menu' : '💡 Suggest'}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                                style={[styles.actionBtn, { backgroundColor: colors.accentSoft, borderRadius: radius.md }]}
                                onPress={() => navigation.navigate('Camera', { mode: 'plate_scan' })}
                            >
                                <Text style={[typography.caption, { color: colors.accent }]}>📷 Scan Plate</Text>
                            </TouchableOpacity>
                            {plate.items.length > 0 && (
                                <TouchableOpacity
                                    style={[styles.actionBtn, { backgroundColor: colors.dangerSoft, borderRadius: radius.md }]}
                                    onPress={handleClear}
                                >
                                    <Text style={[typography.caption, { color: colors.danger }]}>🗑️ Clear</Text>
                                </TouchableOpacity>
                            )}
                        </View>
                        ...
                        <Text style={[typography.label, { color: colors.textMuted }]}>
                            {showRecs ? 'Recommended for You' : 'Canteen Menu'}
                        </Text>
                    </View>
                )}
            />
