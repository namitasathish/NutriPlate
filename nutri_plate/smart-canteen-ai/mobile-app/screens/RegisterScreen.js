...
export default function RegisterScreen({ navigation }) {
    ...
    const handleRegister = async () => {
        if (!username.trim() || !password.trim() || !confirmPassword.trim()) { setError('Please fill in all fields'); return; }
        ...
        try {
            ...
            register({ id: Date.now(), username: username.trim(), role });
        ...
        <View style={[styles.container, { backgroundColor: colors.bg }]}>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.inner}>
                ...
                <View style={[styles.roleContainer, { backgroundColor: colors.cardAlt, borderRadius: radius.xl }]}>
                    <TouchableOpacity style={[styles.roleBtn, role === 'student' && { backgroundColor: colors.accent }, { borderRadius: radius.lg }]} onPress={() => setRole('student')}>
                        <Ionicons name="school" size={14} color={role === 'student' ? '#FFF' : colors.textMuted} style={{ marginRight: 4 }} />
                        <Text style={[typography.bodyBold, { color: role === 'student' ? '#FFF' : colors.textMuted }]}>Student</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.roleBtn, role === 'staff' && { backgroundColor: colors.accent }, { borderRadius: radius.lg }]} onPress={() => setRole('staff')}>
                        <Ionicons name="restaurant" size={14} color={role === 'staff' ? '#FFF' : colors.textMuted} style={{ marginRight: 4 }} />
                ...
                <View style={[styles.inputGroup, { marginTop: spacing.lg }]}>
                    ...
                    <TextInput style={[styles.input, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border, borderRadius: radius.md }]} placeholder="Choose a username" placeholderTextColor={colors.textMuted} value={username} onChangeText={setUsername} autoCapitalize="none" />
                </View>
                <View style={[styles.inputGroup, { marginTop: spacing.md }]}>
                    ...
                    <TextInput style={[styles.input, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border, borderRadius: radius.md }]} placeholder="Create a password" placeholderTextColor={colors.textMuted} value={password} onChangeText={setPassword} secureTextEntry />
                </View>
                <View style={[styles.inputGroup, { marginTop: spacing.md }]}>
                    ...
                    <TextInput style={[styles.input, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border, borderRadius: radius.md }]} placeholder="Confirm your password" placeholderTextColor={colors.textMuted} value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry />
                </View>

                {error ? <Text style={[typography.caption, { color: colors.danger, marginTop: spacing.sm, textAlign: 'center' }]}>{error}</Text> : null}

                <TouchableOpacity style={[styles.registerBtn, { backgroundColor: colors.accent, borderRadius: radius.md, marginTop: spacing.lg }]} onPress={handleRegister} disabled={loading}>
                    {loading ? <ActivityIndicator color="#FFF" /> : <Text style={[typography.bodyBold, { color: '#FFF', fontSize: 16 }]}>Create Account</Text>}
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    roleBtn: { flex: 1, flexDirection: 'row', paddingVertical: 12, alignItems: 'center', justifyContent: 'center' },
